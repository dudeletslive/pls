...
exports = module.exports = function(req, res) {
	...
	var doSignIn = function() {
		...
		var onSuccess = function(user) {
			...
			console.log('------------------------------------------------------------');
			return res.redirect(req.cookies.target || '/my-account');
		}
	...
	var checkExisting = function(next) {
		...
		var query = User.model.findOne();
			...
			query.exec(function(err, user) {
				...
				if (user) {
					console.log('[auth.confirm] - Found existing user via [' + locals.authUser + '] profile id...');
					console.log('------------------------------------------------------------');
					req.flash('success', 'A user has already been created with this ' + locals.authUser.type + ' account. If it was not yet, please sign out.');
